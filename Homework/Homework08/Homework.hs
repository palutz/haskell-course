...
garnacha = "Garnacha"
grapes :: [Grape]
grapes = [sangiovese, cabernet, merlot, garnacha]
...
rjoja = ("Rjoja", "Spain")
regions :: [Region]
regions = [bordeaux, tuscany, rjoja]
...
alcoholPerc :: Kind -> Float
alcoholPerc Red = 14.5
alcoholPerc White = 13
alcoholPerc Rose = 12
...
larrosa :: Label
larrosa = Label [garnacha] rjoja Red
...
castiglioni :: Label
castiglioni = Label [sangiovese] tuscany Red
...
lePetitHaitLafitte = Label [cabernet, merlot] bordeaux Red
...
grapesInLabel :: Grape -> Label -> Bool
grapesInLabel gr lbl = gr `elem` lgrape lbl

containsGrape :: Grape -> [Label] -> Bool
containsGrape agrape = any (grapesInLabel agrape) 

-- This is a test list for the `containsGrape` function with an grape that is not in the list.
grapeList = [larrosa,castiglioni,lePetitHaitLafitte]
newGrape = "Pinot Noir"
